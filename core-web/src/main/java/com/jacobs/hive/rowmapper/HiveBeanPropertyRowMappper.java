 ...
 */
public class HiveBeanPropertyRowMappper<T> implements RowMapper<T> {

    private static Logger logger = LoggerFactory.getLogger(HiveBeanPropertyRowMappper.class);
     ...
     */
    public HiveBeanPropertyRowMappper() {
    }
     ...
     */
    public HiveBeanPropertyRowMappper(Class<T> mappedClass) {
     ...
     */
    public HiveBeanPropertyRowMappper(Class<T> mappedClass, boolean checkFullyPopulated) {
        initialize(mappedClass);
     ...
     * @param tClass the mapped class
     */
    protected void initialize(Class<T> tClass) {
        this.mappedClass = tClass;
            ...
            if (pd != null) {
                try {
                    ...
                    if (rowNumber == 0 && logger.isDebugEnabled()) {
                        logger.debug("Mapping column {} to property {} of type {}", column, pd
                                .getName(), ClassUtils.getQualifiedName(pd.getPropertyType()));
                    }
                    try {
                        ...
                        if (value == null && this.primitivesDefaultedForNullValue) {
                            if (logger.isDebugEnabled()) {
                                logger.debug("Intercepted TypeMismatchException for row {} and "
                                                + "column {}  with null value when setting property {} of "
                                                + "type {} on object: {} ", rowNumber, column, pd.getName(),
