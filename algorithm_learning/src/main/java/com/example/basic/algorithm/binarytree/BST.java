...
public class BST {
   ...
   * 小于node.value，node右子树上的最大搜索二叉树的最小值大于node.value，那么称这整个树都是搜索二叉树。 （2）如果不满足1，则，以node为头的树上的最大搜索二叉树是来自node的左子树上的最大搜索二叉子树和来自node的右子树上的最大搜索二叉子树之间，节点数最大的那个
   * <p> 理解的时候试图递归到最小的单元去一步一步往上推导
   */
  public static Node getBiggestSubBST(Node head, int[] record) {
    if (head == null) {
      record[0] = 0;
    ...
    Node lBST = getBiggestSubBST(left, record);
    int lSize = record[0];
    int lMax = record[1];
    int lMin = record[2];

    Node rBST = getBiggestSubBST(right, record);
    int rSize = record[0];
    int rMax = record[1];
    int rMin = record[2];
  ...
  public static boolean check(Node head) {
    ...
    if (head.left != null) {
      isSearchSubTree = head.value > head.left.value;
    }
    if (head.right != null) {
      isSearchSubTree = head.value < head.right.value;
    }
   ...
   */
  public static boolean isSearchTree2(Node head) {
    if (head == null) {
      return true;
    }
