...
//    rotate(matrix);
    ...
    /**
     * 给定一个double类型的数组arr,其中的元素可正可以负可为0，返回子数组累乘的最大乘积。例如，arr=[-2.5,4,0,3,0.5,8,-1]
     * <p>
     * 子数组[3,0.5,8]累乘可获得最大乘积12，所以返回12
     * <p>
     * 思路：以i结尾的最大子数组乘积可能来自以下三种情况：
     * <p>
     * 1) max*arr[i] , max表示以arr[i-1]结尾的最大子数组乘积，这种情况是有可能的如[3,4,5]
     * <p>
     * 2) min*arr[i], min表示以arr[i-1]结尾的最小子数组乘积
     * <p>
     ...
     * 荷兰国旗问题，三色球问题
     * <p>
     * 给定一个数组arr, 其中只可能含有0，1，2三个值，实现排序：0全在数组左边，1全在数组中间，2全在数组右边
     *
     * @param arr 数组arr
     */
     ...
     * 广度遍历即可
     * <p>
     * 解法：1）开始时生成map矩阵，map[i][j]的含义是从(0,0)位置走到(i,j)位置最短路径值。然后将左上角位置(0,0)的行坐标
     * <p>
     * 与列坐标放入队列rQ，和队列cQ。
     * <p>
     ...
     * <p>
     * 4)在步骤三中，如果一个位置之前走过，就不要重复走，这个逻辑可以根据一个位置在map中的值来确定，比如map[i][j]!=0，就可以知道 这个位置之前已经走过。
     * <p>
     * 5)一直重复步骤2~4。直到遇到右下角位置，说明已经找到终点，返回终点在map中的值即可，如果rQ和cQ已经为空都没有遇到终点位置，说明不存在这样的一条路径，返回0
     * <p>
